...
/**
 * Class Mail
 * @package Magetop\Smtp\Application\Rse
 */
class Mail
{
    /**
     ...
     * @param Data $helper
     */
    public function __construct(Data $helper, ReturnPath $returnPath)
    {
    ...
    public function processMessage($message, $storeId)
    {
        ...
        if ($this->_returnPath[$storeId]) {
            if ($this->smtpHelper->versionCompare('2.2.8')) {
                ...
                $message->getHeaders()->addHeader($this->returnPath->setPath($this->_returnPath[$storeId]));
