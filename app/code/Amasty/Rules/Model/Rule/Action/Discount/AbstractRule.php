...
abstract class AbstractRule extends Discount\AbstractDiscount
{
    ...
    protected function checkSkipRule($rule, $item)
    {
        ...
        $bundleWithSpecialPrice = $item->getProductType() === Type::TYPE_CODE && $item->getProduct()->getSpecialPrice();

        switch ($rule->getAmrulesRule()->getData('skip_rule')) {
            case 0:
                if ($skipSpecialPrice && $bundleWithSpecialPrice) {
                    $result = true;
                    break;
                }
                ...
                break;
            case 1:
                if ($bundleWithSpecialPrice || in_array($item->getProductId(), $this->getItemsWithDiscount($item))) {
                    $result = true;
                }
    ...
    protected function getBaseSumOfItems(array $allItems)
    {
        ...
        foreach ($allItems as $allItem) {
            if ($allItem->getProductType() === Type::TYPE_CODE && $allItem->isChildrenCalculated()) {
                continue;
            }
            $itemBasePrice = $this->validator->getItemBasePrice($allItem);
            if ($allItem->getParentItem() && $allItem->getParentItem()->getProductType() == Type::TYPE_CODE) {
    ...
    protected function isValidChildOfBundle(AbstractItem $item, array $itemsId): bool
    {
        ...
        if ($parentItem
            && ($parentItem->getProductType() === \Magento\Catalog\Model\Product\Type::TYPE_BUNDLE)
