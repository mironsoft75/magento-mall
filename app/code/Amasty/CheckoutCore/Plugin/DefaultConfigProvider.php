...
class DefaultConfigProvider
{
    ...
    public function afterGetConfig(\Magento\Checkout\Model\DefaultConfigProvider $subject, $config)
    {
        ...
        $initRates = $this->checkoutInitialization->getShippingMethods($quote);    
        if(is_array($initRates)){
            $initRates = [$initRates[0]];
        }
        $config['quoteData']['initRates'] = $initRates;
        $config['quoteData']['initPayment'] = $this->checkoutInitialization->getPaymentArray($quote);
        $config['shippingPrice']= \Magento\Framework\App\ObjectManager::getInstance()
        ->get(\Magento\Framework\App\Config\ScopeConfigInterface::class)
        ->getValue('carriers/flatrate/price', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
