...
class Index extends \Magento\Checkout\Controller\Index\Index
{
    ...
    public function execute()
    {
        ...
        if(empty($this->_request->getParam('coupon'))){
            $quote->setCouponCode(null);
        }
        if($quote->getMwRequestedPoints()){
            $quote->setMwRequestedPoints(0)->setUseMwRewardPoints(0)->setMwRwrdpointsAmnt(0);
        }
        $t2 = microtime(true);
        \Magento\Framework\App\ObjectManager::getInstance()->get(\Psr\Log\LoggerInterface::class)->error('log info '.__DIR__.' t2-t1 info : '.($t2-$t1)*1000);
        ...
        $t3 = microtime(true);
        \Magento\Framework\App\ObjectManager::getInstance()->get(\Psr\Log\LoggerInterface::class)->error('log info '.__DIR__.' t3-t2 info : '.($t3-$t2)*1000);
        \Magento\Framework\App\ObjectManager::getInstance()->get(\Psr\Log\LoggerInterface::class)->error('log info '.__DIR__.' t3-t1 info : '.($t3-$t1)*1000);
        return $resultPage;
    }
