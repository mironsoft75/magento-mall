...
class GridProduct extends \Magento\Catalog\Block\Product\AbstractProduct
{
    ...
    public function getLoadedProductCollection()
    {
        ...
        if($parameters){
            ...
            if($type == 'position'){
                $conditionDetails = $conditions->getConditions();
                foreach($conditionDetails as $condit){
                    $attribute = $condit->getAttribute();
                    $operator = $condit->getOperatorForValidate();
                    if('category_ids' == $attribute && '{}' == $operator){
                        $value = $condit->getValue();
                        $collection->getSelect()->join(['cp' => $this->_productCollectionFactory->create()->getTable('catalog_category_product')],
                        "cp.product_id=e.entity_id and cp.category_id in ($value)",['cp_position' => 'position'])->order('cp_position desc');
                    }
                }
            }
  ...
  //   }
    ...
    public function getPositionProducts(){
        ...
        // ->addAttributeToSort('cat_index.position', 'desc')
        ->setPageSize($this->_limit)->setCurPage(1);
