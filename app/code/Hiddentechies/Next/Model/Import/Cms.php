...
namespace Hiddentechies\Next\Model\Import;
...
class Cms
{
    ...
    public function __construct(
        ...
        $this->_importPath = BP . '/app/code/Hiddentechies/Next/etc/import/';
    ...
    public function importCms($type, $demo_version)
    {
        ...
        try {
            ...
            if($this->_scopeConfig->getValue("nextsettings/install/overwrite_".$type)) {
                $overwrite = true;
            }
            ...
            foreach($data['root'][$type]['cms_item'] as $_item) {
                ...
                if($demo_version == "0" || in_array($_item['identifier'],$arr)){
                    if($type == "blocks") {
                        ...
                        if($cms_collection && count($cms_collection) > 0)
                            $exist = true;
